  ...
  <div class="col-xl-12">
    <div class="card">
      <div class="card-header pb-1 mb-0 d-flex align-items-start justify-content-between gap-5">
        <div>
          <h3 class="card-title mb-1">Work Category</h3>
        ...
        <div class="d-flex justify-content-end align-items-center gap-2">
          ...
          @if (auth()->user()->hasPermission('Work Category', 'is_create'))
            <a href="javascript:;" class="btn btn-sm fw-bold btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_sector">
              <span class="me-2"><i class="mdi mdi-plus"></i></span>Add Work Category
            </a>
          @endif
        </div>
      </div>
              ...
              <table class="table align-middle table-row-dashed table-striped table-hover gy-0 gs-1 list_page">
                <thead>
                  <tr class="text-start align-top fw-bold fs-6 gs-0 bg-primary">
                    <th class="min-w-150px">Work Category</th>
                      ...
                      <tr>
                        ...
                        <td>
                          <label class="switch switch-square">
                            <input type="checkbox" class="switch-input" {{ isset($list->status) && $list->status == 0 ? 'checked' : '' }} onchange="statusChange('{{ $list->id }}', this.checked)" {{ auth()->user()->hasPermission('Work Category', 'is_update') ? '' : 'disabled' }} />
                        ...
                        <td>
                          <span class="text-end">
                            @if (auth()->user()->hasPermission('Work Category', 'is_update'))
                            ...
                            @endif
                            @if (auth()->user()->hasPermission('Work Category', 'is_delete'))
    ...
    <div class="modal-dialog modal-md">
      ...
      <div class="modal-content rounded">
        ...
        <div class="modal-header d-flex align-items-center justify-content-between pb-0 border-bottom">
          <h4 class="text-center text-black">Create Work Category</h4>
          <!--begin::Close-->
        ...
        <div class="modal-body py-5 px-10 px-xl-20">
            <div class="row">
                <div class="col-lg-12 mb-3">
                    <label class="text-black mb-1 fs-7 fw-semibold">Work Category Name<span class="text-danger">*</span></label>
                    <input type="text" class="form-control" placeholder="Enter Work Category Name"  id="sector_name" />
        ...
        <div class="modal-footer pt-5">
          <div class="d-flex justify-content-end align-items-center">
            ...
            <button type="button" class="btn btn-primary" onclick="addValidation()" id="addBtn">Create Work Category</button>
          </div>
        </div>
        <!--end::Modal body-->
      </div>
...
<div class="modal fade" id="kt_modal_bulk_upload" tabindex="-1" aria-hidden="true" data-bs-keyboard="false"
    ...
    <div class="modal-dialog modal-md">
      ...
      <div class="modal-content rounded">
        <!--begin::Modal header-->
        <div class="modal-header d-flex align-items-center justify-content-between pb-0 border-bottom">
          <h4 class="text-center text-black">Bulk Upload Work Category</h4>
          <!--begin::Close-->
          <div class="btn btn-sm btn-icon btn-active-color-primary mb-4" data-bs-dismiss="modal">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect opacity="0.5" x="6" y="17.3137" width="16" height="2" rx="1" transform="rotate(-45 6 17.3137)" fill="currentColor" />
...
<div class="modal fade" id="kt_modal_edit_sector" tabindex="-1" aria-hidden="true" data-bs-keyboard="false"
    ...
    <div class="modal-dialog modal-md">
      ...
      <div class="modal-content rounded">
        <!--begin::Modal header-->
        <div class="modal-header d-flex align-items-center justify-content-between pb-0 border-bottom">
          <h4 class="text-center text-black">Update Work Category</h4>
          <!--begin::Close-->
          <div class="btn btn-sm btn-icon btn-active-color-primary mb-4" data-bs-dismiss="modal">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect opacity="0.5" x="6" y="17.3137" width="16" height="2" rx="1" transform="rotate(-45 6 17.3137)" fill="currentColor" />
        ...
        <div class="modal-body py-5 px-10 px-xl-20">
            <div class="row">
                ...
                <div class="col-lg-12 mb-3">
                    <label class="text-black mb-1 fs-7 fw-semibold">Work Category Name<span class="text-danger">*</span></label>
                    <input type="text" class="form-control" placeholder="Enter Work Category Name" id="update_sector_name" />
        ...
        <div class="modal-footer pt-5">
          <div class="d-flex justify-content-end align-items-center">
            <button type="reset" class="btn btn-secondary me-3" data-bs-dismiss="modal">Cancel</button>
            <button type="button" id="edit_appt_btn" class="btn btn-primary" onclick="editValidation()" data-bs-dismiss="modal">Update Work Category</button>
          </div>
        </div>
        <!--end::Modal body-->
      </div>
...
<script>
  function addValidation() {
        ...
        if(reason === '') {
            $('#sector_name_error').text("Work Category Name is required.");
            isValid = false;
        }
    ...
    function createAppointmentReason(data, button) {
        $.ajax({
            url: '{{ url("/settings/work_category/add_work_category") }}',
            method: 'POST',
            data: data,
            dataType: 'json',
            success: function(response) {
                if (response.status === 201 || response.status === 200) {
                    toastr.success(response.message || 'Work Category created successfully!');
                    location.reload();
                } else {
            ...
            complete: function() {
                button.prop('disabled', false).text('Create Work Category');
            }
        });
    }
    ...
    function editDataFetch(id) {
      ...
      $.ajax({
        url: `{{ url("/settings/work_category/edit_work_category") }}/${id}`,
        type: 'GET',
    ...
    function editValidation() {
      ...
      if (reason === "") {
        hasErr = true;
        $('#update_sector_name_error').text("Work Category is required.");
      }
    ...
    function updateAppointmentReason(data, button) {
        $.ajax({
            url: '{{ url("/settings/work_category/update_work_category") }}',
            method: 'POST',
            data: data,
            dataType: 'json',
            success: function(response) {
                if (response.status === 201 || response.status === 200) {
                    toastr.success(response.message || 'Work Category updated successfully!');
                    location.reload();
                } else {
                    toastr.error(response.message || 'An error occurred.');
                }
            ...
            error: function(xhr, status, error) {
                let errorMessage = 'Failed to update Work Category.';
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    errorMessage = xhr.responseJSON.message;
                    if (xhr.status === 422 && xhr.responseJSON.errors) {
                        const errors = xhr.responseJSON.errors;
            ...
            complete: function() {
                button.prop('disabled', false).text('Update Work Category');
            }
        });
    }
    ...
    function statusChange(sno, isChecked) {
        ...
        fetch(`{{ url("/settings/work_category/work_category_status") }}/${sno}`, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.status === 200) {
                toastr.success('Work Category status changed successfully!');
            } else {
                toastr.error('Work Category status changing failed!');
            }
        })
        .catch(error => {
    ...
    function deleteAppointmentReason() {
        ...
        fetch(`{{ url("/settings/work_category/delete_work_category") }}/${domainId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        .then(data => {
            if (data.status === 200) {
                toastr.success('Work Category deleted successfully!');
                location.reload();
            } else {
