        ...
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-3 pb-1">
            ...
            <div class="d-flex align-items-center gap-3">
                <div style="width: 320px; position: relative;">
                    <input type="text" id="teamSearchInput" class="form-control ps-5"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="supervisor_id" id="edit_supervisor_id"
                                    class="form-select select2-team-edit" required>
                                    <option></option>
                                </select>
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Driver <span class="text-danger">*</span></label>
                                <select name="driver_id" id="edit_driver_id" class="form-select select2-team-edit"
                                    required>
                                    <option></option>
                                </select>
                            ...
                            <div class="col-12">
                                <label class="form-label fw-bold">Technicians <span class="text-danger">*</span></label>
                                <select name="technician_ids[]" id="edit_technician_ids"
                                    class="form-select select2-team-edit" multiple required>
                                    <option></option>
                                </select>
                            ...
                            <div class="col-12">
                                <label class="form-label fw-bold">Other Staff (optional)</label>
                                <select name="other_staff_ids[]" id="edit_other_staff_ids"
                                    class="form-select select2-team-edit" multiple>
                                    <option></option>
                                </select>
                            </div>
                        </div>
        ...
        $(document).ready(function() {
            ...
            function initSelect2(selector, modalElement) {
                $(selector).each(function() {
                    ...
                    $(currentElem).select2({
                        ...
                        width: '100%',
                        placeholder: $(currentElem).prop('multiple') ? "Select multiple..." :
                            "Select one...",
                        allowClear: true,
                        ajax: {
                            url: "{{ route('api.users') }}",
                            ...
                            data: function(params) {
                                let selectedIds = [];
                                modalElement.find('.select2-team, .select2-team-edit').each(
                                    function() {
                                        if (this !== currentElem) {
                                            let val = $(this).val();
                                            if (val) {
                                                if (Array.isArray(val)) selectedIds.push(...
                                                    val);
            ...
            $('#addTeamForm').off('submit').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            toastr.success("Team Create successfully ");
                            modal.modal('hide');
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 422) {
                            ...
                            $.each(xhr.responseJSON.errors, function(field, messages) {
                                ...
                                $(`.error-${key}`).text(messages[0]).addClass(
                                    'd-block');
                            });
                        } else {
            ...
            $(document).on('click', '.edit-team-btn', function() {
                ...
                $.get(url, function(res) {
                    if (res.status) {
                        ...
                        setSelect2Val('#edit_supervisor_id', team.supervisor_id, team.supervisor
                            ?.name || 'Supervisor');
                        setSelect2Val('#edit_driver_id', team.driver_id, team.driver?.name ||
                            'Driver');
                        ...
                        if (techId) {
                            setSelect2Val('#edit_technician_ids', techId, team.technician
                                ?.name ||
                                'Technician');
                        }
                        ...
                        if (team.other_staff_ids) {
                            ...
                            let staffs = JSON.parse(team.other_staff_ids) || team
                                .other_staff_ids;
                            if (Array.isArray(staffs)) {
                                staffs.forEach(id => {
                                    setSelect2Val('#edit_other_staff_ids', id,
                                        'Staff ' +
                                        id);
                                });
                            }
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 422) {
                            ...
                            $.each(xhr.responseJSON.errors, function(field, messages) {
                                const key = field.replace('.', '_').replace('[]',
                                    '');
                                $(`.error-edit_${key}`).text(messages[0]).addClass(
                                    'd-block');
                            });
                        } else {
            ...
            $(document).on('click', '.delete-team-btn', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            $(`#team-card-${id}`).fadeOut(300, function() {
                                $(this).remove();
                            });
                        } else {
                    ...
                    error: function() {
                        toastr.error("An error occurred processing your request.");
                        btn.prop('disabled', false);
                    }
                });
            });
