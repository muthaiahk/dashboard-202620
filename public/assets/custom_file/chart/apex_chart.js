	...
	var M = {
			...
			return r(t, [{
				...
				value: function(t) {
					...
					for (var p = t; p < a; p++) {
						...
						if (d) {
							if (s.xaxis.min)
								for (; b < v && r.seriesX[p][b] < s.xaxis.min; b++);
							if (s.xaxis.max)
								for (; v > b && r.seriesX[p][v] > s.xaxis.max; v--);
						}
		...
		q = function() {
			...
			return r(t, [{
				...
				value: function(t) {
					...
					if (i.config.yaxis[t].labels.show) {
						...
						for (var b = function(l) {
								...
								var y = e.axesUtils.getYAxisForeColor(s.colors, t),
									w = x.listToArray(i.globals.dom.baseEl.querySelectorAll(".apexcharts-yaxis[rel='".concat(t, "'] .apexcharts-yaxis-label tspan"))).map((function(t) {
								...
								if (A.textContent = Array.isArray(b) ? b.join(" ") : b, k.node.appendChild(A), 0 !== i.config.yaxis[t].labels.rotate) {
									var S = a.rotateAroundCenter(p.node),
										C = a.rotateAroundCenter(k.node);
					...
					var A = i.config.yaxis[t].axisBorder,
						S = 31 + A.offsetX;
		...
		nt = function() {
			...
			return r(t, [{
				...
				value: function() {
					var t = this.w.globals.dom.baseEl.querySelector(".apexcharts-legend"),
						e = t.offsetWidth;
					return {
						clwh: t.offsetHeight,
						clww: e
		...
		lt = function() {
			...
			return r(t, [{
				...
				value: function(t) {
					...
					var l = Array.isArray(s.config.legend.markers.size) ? parseFloat(s.config.legend.markers.size[i]) : parseFloat(s.config.legend.markers.size),
						h = Array.isArray(s.config.legend.markers.offsetX) ? parseFloat(s.config.legend.markers.offsetX[i]) : parseFloat(s.config.legend.markers.offsetX),
						c = Array.isArray(s.config.legend.markers.offsetY) ? parseFloat(s.config.legend.markers.offsetY[i]) : parseFloat(s.config.legend.markers.offsetY),
						d = Array.isArray(s.config.legend.markers.strokeWidth) ? parseFloat(s.config.legend.markers.strokeWidth[i]) : parseFloat(s.config.legend.markers.strokeWidth),
						g = r.style;
					if (g.height = 2 * (l + d) + "px", g.width = 2 * (l + d) + "px", g.left = h + "px", g.top = c + "px", s.config.legend.markers.customHTML) g.background = "transparent", g.color = a[i], Array.isArray(s.config.legend.markers.customHTML) ? s.config.legend.markers.customHTML[i] && (r.innerHTML = s.config.legend.markers.customHTML[i]()) : r.innerHTML = s.config.legend.markers.customHTML();
						...
						var u = new D(this.ctx).getMarkerConfig({
							...
							p = SVG(r).size("100%", "100%"),
							f = new m(this.ctx).drawMarker(0, 0, e(e({}, u), {}, {
				...
				value: function() {
					...
					var e = this.legendHelpers.getLegendDimensions(),
						i = new ot(this.ctx),
			...
			return r(t, [{
				...
				value: function(t) {
					...
					return L = this.barCtx.isHorizontal ? this.calculateBarsDataLabelsPosition(Y) : this.calculateColumnsDataLabelsPosition(Y), b.attr({
						...
						x: L.dataLabelsX,
						y: L.dataLabelsY,
						val: this.barCtx.isRangeBar ? [r, o] : "100%" === v.config.chart.stackType ? d[n][l] : v.globals.series[n][l],
						i: h,
						j: l,
						barWidth: u,
						barHeight: g,
				...
				value: function(t) {
					...
					var r, o, n = t.y,
						...
						w = l;
					c = Math.abs(c);
					var k = "vertical" === e.config.plotOptions.bar.dataLabels.orientation,
				...
				value: function(t) {
					...
					for (var b = f ? t.series.length - 1 : 0; f ? b >= 0 : b < t.series.length; f ? b-- : b++) {
						...
						if (s.setSelectionFilter(I, 0, b), this.addListeners(I, this.radialDataLabels), y.add(I), I.attr({
							...
							var F = z.x + this.barLabels.offsetX,
								R = z.y + this.barLabels.offsetY,
								D = i.drawText({
									x: F,
									y: R,
									text: X,
