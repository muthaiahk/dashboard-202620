...
class TemplateCustomizer {
  constructor({
    ...
    this.container = document.querySelector("#template-customizer");

    if (!this.container) {
        console.warn("Template customizer container not found");
        return;
    }
    this._setup()
  }
  _setup() {
    console.log('Template customizer setup completed.')
  }
...
_showResetBtnNotification(t = true) {
      setTimeout(() => {
          const badge = this.container?.querySelector(".template-customizer-reset-btn .badge");
          if (!badge) return;

          badge.classList.toggle("d-none", !t);
      }, 200);
  }
